...
function updater() {
    ...
    if (sessionStorage.getItem("id_name") && sessionStorage.getItem("phone_number") && sessionStorage.getItem("user_address")) {
    ...
    let qrCodeValue = sessionStorage.getItem("qr_code");
    if (qrCodeValue) {
        // Only regenerate QR if it changed
        if (qrImg.dataset.qrValue !== qrCodeValue) {
            ...
            if (canvas) {
                ...
                qrImg.dataset.qrValue = qrCodeValue;
            }
        }
    }
}
