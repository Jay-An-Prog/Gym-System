...
import { syncStatusToStoredQRS } from "./esp32.js";
import { syncSubsExpDateToStoredQRS } from "./esp32.js";
...
if (memberId) {
    ...
    onValue(statusRef, async (snapshot) => {
        ...
        const previousStatus = sessionStorage.getItem("status");

        // Update sessionStorage
        sessionStorage.setItem("status", status);
        console.log("Realtime status updated:", status);

        // 🔥 Only update ESP32 related if status changed
        status !== previousStatus 
            ? await syncStatusToStoredQRS(status) 
            : console.log("Status unchanged, skipping sync.");
...
function initializeQrCheckinRT() {
    ...
    onValue(qrCheckinRef, (snap) => {
        ...
        if (checkin && checkin > 0) {
            sessionStorage.setItem("qr_checkin", new Date(checkin).toISOString());
        } else {
...
async function updateUserInfo() {
    ...
    try {
        ...
        if (sessionStorage.getItem("status") === "activated") {
            ...
            sessionStorage.setItem("receipt_image_url", subscriptionData.receipt_image_url || "");
            if (subscriptionData.expiration_date?.toDate) {
                ...
                // 🔥 Only update ESP32 related
                await syncSubsExpDateToStoredQRS(Math.floor(subscriptionData.expiration_date.toDate().getTime() / 1000));
            } else {
                sessionStorage.setItem("expiration_date", "");

                // 🔥 Only update ESP32 related
                await syncSubsExpDateToStoredQRS(0);
            }
