...
if (memberId) {
    ...
    onValue(statusRef, async (snapshot) => {
        ...
        if (
            ...
            initializeQrRealtime();
        }
    });
}
function initializeQrRealtime() {
    const currentQr = sessionStorage.getItem("qr_code");

    const qrRef = rtdbRef(
        rtdb,
        `stored_qrs/${currentQr}`
    );

    onValue(qrRef, (snap) => {
        const data = snap.val() || {};

        const checkin = data.qr_checkin;
        const checkout = data.qr_checkout;

        sessionStorage.setItem("qr_checkin", checkin && checkin > 0 ? checkin : "");
        sessionStorage.setItem("qr_checkout", checkout && checkout > 0 ? checkout : "");

        console.log("Realtime QR updated:", { checkin, checkout });
    });
}
