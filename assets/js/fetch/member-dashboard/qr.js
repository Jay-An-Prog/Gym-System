...
generateBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        if (oldQr) {
            const oldCheckinRef = ref(rtdb, `stored_qrs/${oldQr}/qr_checkin`);
            const oldCheckinSnap = await get(oldCheckinRef);
            const oldCheckin = oldCheckinSnap.exists() ? oldCheckinSnap.val() : 0;
        
            // Block generation if qr_checkin has a timestamp
            if (oldCheckin !== 0) {
                modalMsg("You already have an active check-in. Please wait for the check-in to reset.");
                return;
            }
        
            // Safe to delete old QR only if qr_checkin is 0
