...
import { storage, BUCKET_ID, ID } from '../../utils/appwrite.js';
...
document.getElementById("uploadButton").addEventListener("click", async () => {
    ...
    let receiptFileId = null; // 🔁 Track uploaded file for rollback

    try {
        // 🗜️ Compress image (any size → optimized)
        const compressedFile = await compressFileToFile(file);

        // 📁 Rename with virtual folder path
        ...
        receiptFileId = upload.$id; // Save file ID for possible rollback
        ...
        // 🔁 Rollback: delete uploaded file if Firestore failed
        if (receiptFileId) {
            try {
                await storage.deleteFile(BUCKET_ID, receiptFileId);
                console.warn("Rollback successful: uploaded file removed.");
            } catch (cleanupError) {
...
// ===================================================
// 🗜️ IMAGE COMPRESSION (FILE → FILE)
// ===================================================
function compressFileToFile(file, maxWidth = 480, maxHeight = 480, quality = 0.6) {
