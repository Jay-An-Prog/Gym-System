...
uploadForm.addEventListener("submit", async (e) => {
    ...
    if (sessionStorage.getItem("status") === "activated") {
        confirmRequest = confirm("Caution: Updating your information will remove your active status until it is reviewed by the admin. Do you want to continue?");
        if (!confirmRequest) return;
    }
    ...
    try {
        ...
        if (memberDoc.exists()) {
            ...
            if (data.face_file_id) {
                try { await storage.deleteFile(BUCKET_ID, data.face_file_id); } 
                catch(e){ console.warn("Old face file delete failed:", e); }
            }
            
            if (data.id_file_id) {
                try { await storage.deleteFile(BUCKET_ID, data.id_file_id); } 
                catch(e){ console.warn("Old ID file delete failed:", e); }
            }
        }
