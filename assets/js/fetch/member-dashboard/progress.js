...
async function getAIOutputFromFirestore() {
    ...
    if (snapshot.exists()) { // Only this oart executes when nutrition AI is used
    const { progress_output } = snapshot.data();

        if (progress_output) {
            sessionStorage.setItem(WORKOUT_AI_CACHE_KEY, progress_output);
            sessionStorage.setItem(WORKOUT_AI_UPDATED_KEY, Date.now());
            console.log("AI output loaded from firestore");
        } else {
            sessionStorage.setItem(WORKOUT_AI_CACHE_KEY, "Save your workout progress and analyze with AI Coaching");
        }
    }
}
...
// ------------------------------
// INITIAL LOAD ATTENDANCE SUMMARY
// ------------------------------
