...
import { setPersistence, browserLocalPersistence, signInWithPopup, signInWithRedirect, getRedirectResult, signOut } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-auth.js";
...
async function handleUser(user) {
    ...
    if (email.toLowerCase().endsWith("@sonic.com")) {
        console.log("Sonic account detected. Auto signout.");
        ...
        sessionStorage.setItem("portal_loading_box", "hide"); // hide loading box
        return;
    }
    ...
    } else { // Handle user that has a deactivated account   
        const statusData = statusSnap.val();    
        if (statusData.status === "deactivated") {
            alert("Your account has been deactivated. Please contact the gym staff.");
