...
function updater() {
    ...
    if (sessionStorage.getItem("id_name") && sessionStorage.getItem("phone_number") && 
        ...
        document.getElementById("defaultPic").classList.add("hidden");
        document.getElementById("userPic").classList.remove("hidden");

        // ✅ Retrieve back from sessionStorage (proves it's saved properly)
        const savedImgBytes = localStorage.getItem("face_image_bytes");
  
        // ✅ Display stored image
        const faceDisplay = document.getElementById("userPic");
        faceDisplay.innerHTML = "";
  
        const img = document.createElement("img");
        img.src = savedImgBytes;    // Use stored Base64
        img.alt = "Face Image";
        ...
        document.getElementById("defaultPic").classList.remove("hidden");
        document.getElementById("userPic").classList.add("hidden");
