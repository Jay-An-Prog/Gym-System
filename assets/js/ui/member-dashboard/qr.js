...
setInterval(() => {
    let checkinTimestamp = Number(sessionStorage.getItem("qr_checkin"));
    if (checkinTimestamp && sessionStorage.getItem("status") === "activated" && sessionStorage.getItem("subscription_status") === "active") { // safety check   

        if (checkinTimestamp) {
            if (checkinTimestamp < 1e12) checkinTimestamp *= 1000; // convert seconds → ms
            let checkinDate = new Date(checkinTimestamp);
            let formattedTime = checkinDate.toLocaleTimeString();
            document.getElementById("checkinTime").textContent = formattedTime;
        } else {
            document.getElementById("checkinTime").textContent = "No check-in time available";
        }    
