...
function hideForm() { // Can be called in nutrition.js fetch to render AI Output
    document.getElementById("viewInfo").style.display = "block";
    document.getElementById("hideInfo").style.display = "none";
    document.getElementById("nutritionForm").style.display = "none";
...
function updateAIOutputUI() {
    ...
    const stamp = sessionStorage.getItem(NUTRITION_AI_UPDATED_KEY);
    if (stamp === lastAIStamp) return;

    lastAIStamp = stamp;

    const output = sessionStorage.getItem(NUTRITION_AI_CACHE_KEY);
